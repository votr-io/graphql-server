...
export async function upsertElection(ctx: Context, election: Election) {
  const { id, createdBy, name, description, status, voteCount } = election;
  ...
  const query = sql`
    INSERT INTO elections (id, date_created, date_updated, created_by, name, description, candidates, status, vote_count, results)
    VALUES (${id}, ${dateCreated}, ${dateUpdated}, ${createdBy}, ${name}, ${description}, ${candidates}, ${status}, ${voteCount}, ${results})
    ON CONFLICT (id)
